			...
			if(!empty($cek_nik))
			{
					...
					<h3 class="box-title">TABLE JENIS DOKUMEN</h3>
						<div class="pull-right">
							<table>
								<tr>
									<td>
										<input type="text" id="cari_layanan" name="cari_layanan"  maxlength="35" class="text form-control" size="35" alt="tlp" title="Cari data" placeholder="...Cari" value="" />
									</td>
									<td>
										<a href="javascript:void(0)" class="btn btn-default btn-sm btn-flat btn-block" onclick="view_layanan()" title = "Cari Dokumen" alt = "Cari Dokumen" style="border: 1px dotted blue;">CARI LAYANAN</a>
									</td>
								</tr>
							</table>
						</div>
					</div>
					<div class="box-body" id="table_layanan">
					';
				...
				if(!empty($list_layanan))
				{
					echo'<table width="100%" id="example2" class="table table-hover hoverTable" style="opacity:1;">';
						echo'<thead>';
						echo'<tr>';
													echo '<th width="5%" style="text-align:center;background-color:red;color:white;font-weight:bold;border: 1px solid black;border-collapse: separate;">No</th>';
													echo '<th width="20%" style="text-align:center;background-color:red;color:white;font-weight:bold;border: 1px solid black;border-collapse: separate;">NAMA PELAYANAN</th>';
													echo '<th width="20%" style="text-align:center;background-color:red;color:white;font-weight:bold;border: 1px solid black;border-collapse: separate;">TAHAPAN</th>';
													echo '<th width="20%" style="text-align:center;background-color:red;color:white;font-weight:bold;border: 1px solid black;border-collapse: separate;">PERSYARATAN</th>';
													echo '<th width="30%" style="text-align:center;background-color:red;color:white;font-weight:bold;border: 1px solid black;border-collapse: separate;">DATA</th>';
													echo '<th width="5%" style="text-align:center;background-color:red;color:white;font-weight:bold;border: 1px solid black;border-collapse: separate;">AKSI</th>';
						echo'</tr>';
						echo'</thead>';
...
<a href="javascript:void(0)" class="btn btn-danger btn-sm btn-flat btn-block" id="btn-'.$row->id_jenis_naskah.'-'.$no.'" onclick="view_history_ajuan(this)" title = "Ubah Data '.$row->nama_jenis_naskah.'" alt = "Ubah Data '.$row->nama_jenis_naskah.'">LIHAT AJUAN ('.$row->CNT.')</a>
		...
		{
			$cari_layanan = htmlentities($_POST['cari_layanan'], ENT_QUOTES, 'UTF-8');
			$nik = htmlentities($_POST['nik'], ENT_QUOTES, 'UTF-8');
			$kode_kantor = htmlentities($_POST['kode_kantor'], ENT_QUOTES, 'UTF-8');
			...
			$query_list_layanan = "SELECT
						...
						(
							...
							(
								...
								LEFT JOIN tb_tahapan AS C 
									ON A.id_tahapan  = C.id_tahapan  
						...
						(
							...
							(
								...
								LEFT JOIN tb_jenis_naskah AS B 
									ON A.id_jenis_naskah = B.id_jenis_naskah 
									AND A.kode_kantor = B.kode_kantor
								LEFT JOIN tb_persyaratan AS C 
									ON A.id_syarat  = C.id_syarat  
									AND A.kode_kantor = C.kode_kantor
								WHERE A.kode_kantor = '".$kode_kantor."'
								-- WHERE COALESCE(B.nama_jenis_naskah,'') LIKE '%%'
							) AS A2
						...
						WHERE COALESCE(A.nama_jenis_naskah,'') LIKE '%".$cari_layanan."%' 
...
<a href="javascript:void(0)" class="btn btn-danger btn-sm btn-flat btn-block" id="btn-'.$row->id_jenis_naskah.'-'.$no.'" onclick="view_history_ajuan(this)" title = "Ubah Data '.$row->nama_jenis_naskah.'" alt = "Ubah Data '.$row->nama_jenis_naskah.'">LIHAT AJUAN ('.$row->CNT.')</a>
					...
					echo'<table width="100%" id="example2" class="table table-hover hoverTable" style="opacity:1;">';
						...
						echo'<tr>';
							echo'<td colspan="6" style="text-align:center;">';
								echo'PENCARIAN TIDAK DITEMUKAN DATA';
							echo'</td>';
						echo'</tr>';
					echo'</table>';
				}
		...
		function view_pengajuan()
		{
			...
			if(!empty($cek_nik))
			{
				...
				if(!empty($cek_jenis_naskah))
				{
					...
					$query_cek_apakah_sudah_ada_edit = "
													SELECT * 
													FROM tb_pengajuan 
				...
				{
					//return false;
					echo'JENIS NASKAH TIDAK DITEMUKAN';
				}
			}
			else
			{
		...
		function simpan_pengajuan_pelayanan()
		{
			...
			else
			{
				...
				$query_penduduk = "SELECT * FROM tb_penduduk WHERE nik = '".$_POST['sumber']."' AND kode_kantor = '".$kode_kantor."';";
					...
					foreach($list_result as $row)
					{
						echo'<tr>';
							...
							if($row->hasil_pengajuan == '')
							{
								...
								$nb = '<div style="padding:2%;border:1px dotted black;color:red;">
									NB : Dokumen bisa dicetak jika sudah diperiksa dan acc admin
								<div>';
							}
							else
							{
...
<a href="javascript:void(0)" class="btn btn-default btn-sm btn-flat btn-block" id="btn-'.$row->id_jenis_naskah.'-'.$no.'-qr" title = "Cetak Data '.$row->nama_jenis_naskah.'" alt = "Cetak Data '.$row->nama_jenis_naskah.'" onclick="cetak_qr_view_langsung_print(this)"> <span class="glyphicon glyphicon-print " style="padding-top:1.5%;"></span> CETAK QR</a>
...
<a href="javascript:void(0)" class="btn btn-default btn-sm btn-flat btn-block" id="btn-'.$row->id_jenis_naskah.'-'.$no.'-progress" title = "Cetak Data '.$row->nama_jenis_naskah.'" alt = "Cetak Data '.$row->nama_jenis_naskah.'" onclick="cek_progress(this)"> <span class="glyphicon glyphicon-hourglass " style="padding-top:1.5%;"></span> PROGRES</a>
		...
		function view_download_dok()
		{
			...
			if(!empty($get_pengajuan))
			{
				...
				//GET PENDUDUK
				$query_get_data_penduduk = "
										SELECT * 
										FROM tb_penduduk 
										WHERE nik = '".$get_pengajuan->sumber."'
										AND kode_kantor = '".$kode_kantor."'
									";
				...
				if(!empty($get_data_penduduk))
				{
					...
					if(!empty($get_jenis_naskah))
					{
						...
						$query_get_isian_naskah = "
							...
							LEFT JOIN tb_var_naskah AS B 
								ON A.id_jenis_naskah = B.id_jenis_naskah
								AND A.kode_kantor = B.kode_kantor
								AND A.id_var_naskah = B.id_var_naskah 
								AND B.id_jenis_naskah = '".$get_jenis_naskah->id_jenis_naskah."'
		...
		function cek_satu_nik_satu_hari_ajuan()
		{
			...
			$query = "
						...
						WHERE id_jenis_naskah = '".$id_jenis_naskah."' 
								AND sumber = '".$nik."' 
								AND DATE(tgl_surat_masuk) = DATE(NOW())
								AND kode_kantor = '".$kode_kantor."'
								;";
		...
		function cetak_print_qr_code()
		{
			...
			if(!empty($get_data_pengajuan))
			{
				...
				$data = array('get_data_pengajuan'=>$get_data_pengajuan);
				$this->load->view('admin/page/king_admin_print_qr.php',$data);
			}
			else
			{
				echo'TIDAK ADA DATA YANG DITAMPILKAN';
		...
		function cetak_tagihan()
		{
			//$nik = htmlentities($_POST['nik'], ENT_QUOTES, 'UTF-8');
			//$nik = $_GET['nik'];
			$nik = htmlentities($_GET['nik'], ENT_QUOTES, 'UTF-8');
			$kode_kantor = htmlentities($_GET['kode_kantor'], ENT_QUOTES, 'UTF-8');
			
			$jsonobj = $this->get_json_pajak($nik);
			
			$tgl = $this->tanggal(date('Y-m-d'));
			$data = array('jsonobj'=>$jsonobj, 'nik' => $nik,'tgl'=>$tgl,'kode_kantor'=>$kode_kantor);
